#!/usr/bin/env python3
"""
Test Groq-Powered Job Analysis
Demonstrates the new Groq AI-powered job description analysis
"""

from application_system import application_system
from setup_groq import load_groq_api_key, test_groq_connection
import json

def test_groq_job_analysis():
    """Test Groq-powered job analysis with a real job"""
    
    print("🤖 Testing Groq-Powered Job Analysis")
    print("=" * 50)
    ...
    if groq_api_key:
        ...
        if not test_groq_connection(groq_api_key):
            print("❌ Groq API connection failed, will use fallback")
            groq_api_key = None
    else:
        print("⚠️ No Groq API key found, will use fallback analysis")
    ...
    print(f"\n🤖 Running Groq AI-powered job analysis...")
    print("-" * 40)
    ...
    try:
        ...
        print(f"\n📝 Testing Groq AI-powered resume tailoring...")
        print("-" * 40)
        ...
        print(f"\n🎉 Groq AI-powered analysis completed successfully!")
        print(f"💡 The system now:")
        print(f"   - Fetches real job descriptions from URLs")
        print(f"   - Uses Groq AI to extract skills and requirements")
        ...
        if groq_api_key:
            print(f"🚀 Using Groq AI for advanced analysis")
        else:
            print(f"🔄 Using fallback keyword matching")
        
    except Exception as e:
        print(f"❌ Error during Groq AI analysis: {e}")
        print(f"💡 This might be due to:")
        print(f"   - Network issues fetching job description")
        print(f"   - Job URL not accessible")
        print(f"   - Groq API issues (will use fallback)")

if __name__ == "__main__":
    test_groq_job_analysis() 